  ...
  for (i = 0; i < nConvFilters_2; i++) {
    ...
    for (l = 0; l < nConvFilters_1; l++) {
      if (keepers.e(l + 1, i + 1)) {
        ...
        for (j = 0; j < filterSize_1; j++) {
          for (k = 0; k < filterSize_1; k++) {
            conv_weights_2a[i][l][j][k] = conv_nodes[1][i][keeperCount].e(
              j + 1,
              k + 1
            );
          }
...
function getNNOutput() {
  ...
  for (f = 0; f < nConvFilters_2; f++) {
    ...
    for (i = halfFilter; i < inputImageSize - halfFilter; i++) {
      for (j = halfFilter; j < inputImageSize - halfFilter; j++) {
            ...
            for (m = -halfFilter; m <= halfFilter; m++) {
              for (n = -halfFilter; n <= halfFilter; n++) {
                outputImage[i][j] =
                  outputImage[i][j] +
                  image[i + m][j + n] *
                    conv_nodes[convLayer][f][keeperCount].e(
                      m + halfFilter + 1,
                      n + halfFilter + 1
                    );
              }
        ...
        allNodeNums[count] =
          count - nPixels - nConvNodes_1 - nConvNodes_1_down - 10 * 10 * f + 1;
        count++;
      }
    }
  }
    ...
    for (i = 0; i < downMapSize; i++) {
      for (j = 0; j < downMapSize; j++) {
        ...
        allNodeNums[count] =
          ...
          1;
        inputArray[
          count - nPixels - nConvNodes_1 - nConvNodes_1_down - nConvNodes_2
        ] = downMaps_2[f][j][i];
        count++;
      }
    }
  ...
  for (i = 1; i <= nHiddenNodes_2; i++) {
    if (!allZeroes) {
      ...
      allNodeOutputs[fcCountStart + nHiddenNodes_1 + i - 1] =
        hidden_outputs_2a[i - 1];
    } else {
  ...
  for (i = 1; i <= nFinalNodes; i++) {
    if (!allZeroes) {
      ...
      allNodeInputs[fcCountStart + nHiddenNodes_1 + nHiddenNodes_2 + i - 1] =
        newSums.e(i);
      allNodeOutputs[fcCountStart + nHiddenNodes_1 + nHiddenNodes_2 + i - 1] =
        final_outputsa[i - 1];
    } else {
      final_outputsa[i - 1] = 0;
      allNodeInputs[fcCountStart + nHiddenNodes_1 + nHiddenNodes_2 + i - 1] = 0;
      allNodeOutputs[
        fcCountStart + nHiddenNodes_1 + nHiddenNodes_2 + i - 1
      ] = 0;
    }
  ...
  if (!allZeroes) {
    ...
    finalOutputID =
      fcCountStart + nHiddenNodes_1 + nHiddenNodes_2 + i - 1 + ind1 - 10;
...
function normalizeWithinLayer(arr) {
  ...
  for (i = 0; i < len; i++) {
    if (i < nPixels) {
      if (arr[i] > maxPixel) maxPixel = arr[i];
      else if (arr[i] < minPixel) minPixel = arr[i];
    } else if (i < nPixels + nConvNodes_1) {
      for (j = 1; j <= nConvFilters_1; j++) {
        if (i >= nPixels + 28 * 28 * (j - 1) && i < nPixels + 28 * 28 * j) {
          if (arr[i] > maxConv1[j - 1]) maxConv1[j - 1] = arr[i];
          else if (arr[i] < minConv1[j - 1]) minConv1[j - 1] = arr[i];
        }
      }
    } else if (i < nPixels + nConvNodes_1 + nConvNodes_1_down) {
      for (j = 1; j <= nConvFilters_1; j++) {
        if (
          i >= nPixels + nConvNodes_1 + 14 * 14 * (j - 1) &&
          i < nPixels + nConvNodes_1 + 14 * 14 * j
        ) {
          if (arr[i] > maxConv1_down[j - 1]) maxConv1_down[j - 1] = arr[i];
          else if (arr[i] < minConv1_down[j - 1]) minConv1_down[j - 1] = arr[i];
        }
      }
    } else if (i < nPixels + nConvNodes_1 + nConvNodes_1_down + nConvNodes_2) {
      for (j = 1; j <= nConvFilters_2; j++) {
        if (
          i >= nPixels + nConvNodes_1 + nConvNodes_1_down + 10 * 10 * (j - 1) &&
          i < nPixels + nConvNodes_1 + nConvNodes_1_down + 10 * 10 * j
        ) {
          if (arr[i] > maxConv2[j - 1]) maxConv2[j - 1] = arr[i];
          else if (arr[i] < minConv2[j - 1]) minConv2[j - 1] = arr[i];
        }
      }
    } else if (
      i <
      ...
      if (arr[i] > maxHidden1) maxHidden1 = arr[i];
      else if (arr[i] < minHidden1) minHidden1 = arr[i];
    } else if (
      i <
      nPixels +
        nConvNodes_1 +
      ...
      if (arr[i] > maxHidden2) maxHidden2 = arr[i];
      else if (arr[i] < minHidden2) minHidden2 = arr[i];
    } else {
      if (arr[i] > maxFinal) maxFinal = arr[i];
      else if (arr[i] < minFinal) minFinal = arr[i];
    }
  }
    ...
    for (i = 0; i < len; i++) {
      if (i < nPixels) {
        ...
        for (j = 1; j <= nConvFilters_1; j++) {
          if (i >= nPixels + 28 * 28 * (j - 1) && i < nPixels + 28 * 28 * j) {
            arr[i] =
              (arr[i] - minConv1[j - 1]) / (maxConv1[j - 1] - minConv1[j - 1]);
          }
        }
      } else if (i < nPixels + nConvNodes_1 + nConvNodes_1_down) {
        for (j = 1; j <= nConvFilters_1; j++) {
          if (
            ...
            arr[i] =
              (arr[i] - minConv1_down[j - 1]) /
              (maxConv1_down[j - 1] - minConv1_down[j - 1]);
          }
        }
      } else if (
        i <
        nPixels + nConvNodes_1 + nConvNodes_1_down + nConvNodes_2
      ) {
        for (j = 1; j <= nConvFilters_2; j++) {
          if (
            i >=
              nPixels + nConvNodes_1 + nConvNodes_1_down + 10 * 10 * (j - 1) &&
            i < nPixels + nConvNodes_1 + nConvNodes_1_down + 10 * 10 * j
          ) {
            arr[i] =
              (arr[i] - minConv2[j - 1]) / (maxConv2[j - 1] - minConv2[j - 1]);
          }
        }
      } else if (
        i <
...
function maxPool(inputMap, poolSize, inputImageSize) {
  ...
  for (i = prevHalfFilter; i <= inputImageSize - prevHalfFilter; i += stride) {
    ...
    for (
      j = prevHalfFilter;
      j <= inputImageSize - prevHalfFilter;
      j += stride
    ) {
      var max = -100;
