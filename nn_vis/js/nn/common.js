...
function updateInfoBoxPos() {
  ...
  var bot = Math.min(
    window.innerHeight - mousepx.y + 20,
...
function drawFilter() {
  ...
  if (layerNum[interID] == 0) {
      ...
      if (keepers.e(f + 1, filterNum + 1)) {
        for (i = 0; i < filterSize_1; i++) {
          for (j = 0; j < filterSize_1; j++) {
            ...
            filterCtx.fillStyle = "rgba(" + r + "," + g + "," + b + ", 1.0";
            filterCtx.fillRect(
              reverseKeeperCount * squareWidth + i * pixelSize,
              j * pixelSize,
              pixelSize,
              pixelSize
            );
    ...
    for (j = 1; j <= nConvNodes_1_down; j++) {
      ...
      temp_below = allNodeNums[ind_below] - 1;
      filterNum_below = math.floor(
        (ind_below - nPixels - nConvNodes_1) / (14 * 14)
      );
      ...
      if (keepers.e(filterNum_below + 1, filterNum + 1) == 1) {
        if (
          ...
          inputCtx.fillStyle = "rgba(" + r + "," + g + "," + b + ", 1.0";
          inputCtx.fillRect(
            keeperCount * squareWidth + x * pixelSize,
            y * pixelSize,
            pixelSize,
            pixelSize
          );
